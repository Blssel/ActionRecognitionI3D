...
CACHE_DIR='./jiayiceng_cache_test'
...
def get_bottleneck_tensor_by_indexs(sess,n_classes,video_lists,label_index,video_index,category, npy_data_tensor, bottleneck_tensor):
	flag=True
	label_name = list(video_lists.keys())[label_index]#获得这个类名
	#计算bottleneck的值!!!!!
	bottleneck,flag = get_or_create_bottleneck(sess, video_lists, label_name, video_index, category, npy_data_tensor, bottleneck_tensor)
	if flag==False:
		...
		#print('format is not suitable,pass!----%s'%label_name)
		return bottleneck, ground_truth, flag
	#生成这个类对应的groundtruth编码（维度就是类别数）
	ground_truth = np.zeros([1,n_classes], dtype=np.float32)#数据类型是float32型
	ground_truth[0][label_index] = 1.0#让这个类的索引对应位的值取1
	#bottleneck和groundtruth并入batch中
    ...
    if not os.path.exists(bottleneck_path):
		...
		try:
			...
			return bottleneck_values,flag
                #中央crop
                npy_data=npy_data[0:1,0:80,0:224,int((npy_data.shape[3]-224)/2):int((npy_data.shape[3]-224)/2)+224,0:3]
		#取前80帧
                count=0
		while len(npy_data[0])<80:
                        count+=1
                        ...
                        try:
                                ...
                                bottleneck_values=np.array([])
                                return bottleneck_values,flag
                        npy_data=npy_data[0:1,0:80,0:224,0:224,0:3]
                        if count>=4:
                            break

		if npy_data.shape!=(1,80,224,224,3):
                        print('video wrong')
			flag=False
			bottleneck_values=np.array([])
			return bottleneck_values,flag
